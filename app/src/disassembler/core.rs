...
impl<R: Read> Disassembler<ReaderSource<R>> {
    pub fn from_reader(reader: R, addr: u64, buf_size: usize) -> Result<Self> {
        Self::with_source(ReaderSource(reader), addr, buf_size)
    }

    pub fn disasm_all(self) -> Result<DisasmStream<R>> {
        DisasmStream::new(self.source.0, self.addr, self.buf_size)
    }
}
...
impl<'a> Disassembler<MemorySource<'a>> {
    ...
    pub fn disasm_from_to_addr(&self, start_addr: u64, end_addr: u64) -> Result<DisasmStream<Take<Cursor<&'a [u8]>>>> {
        let mut reader = Cursor::new(self.source.0);
