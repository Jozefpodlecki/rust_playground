...
pub fn read_settings(settings_file_name: &Path) -> Result<Settings> {
...
pub fn create_settings_if_not_exists(settings_file_name: &Path) -> Result<Arc<RwLock<Settings>>> {
...
pub fn watch<P: AsRef<Path>>(path: P, settings: Arc<RwLock<Settings>>) -> Result<()> {
